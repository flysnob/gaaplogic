$('document').ready(function(){
	...
	$('#cancel_override_2').click(function(){
		r = false;
		if ($('#stop_flag').text() == '1'){
			r = confirm('Canceling override of the delivery date and/or delivery status will reset the revenue recognition method analysis. Proceed?');
		} 
		if (r == true){
			var url = path + 'json_rr_update.php';
	...
	$('.save-a').click(function(){
		...
		if (statusId == '2'){ 													// if this is the delivery condition
			r = false;
			if ($('#stop_flag').text() == '1'){
				r = confirm('Changing the delivery date or the delivery status will reset revenue recognition method analysis. Proceed?');
			}
			if (r == true){	
				var url = path + 'json_rr_update.php';
				var resetData = { 'project_id' : projectId, 'meth_cat_id' : '2' }
		...
		if (r == true){
			...
			for (var i = 0; i < delivResults.length; i++){
				if (delivResults[i].rr_del_id == rrDelivId){
					...
					delDesc = '<strong>' + delivResults[i].rr_del_desc + '</strong>'; 
			...
			$('#element_rr_meth_desc').html('<strong></strong>');
		}
		overall_status();
	})
	...
	jQuery.extend({
		saveData: function(url, data) { // saveData method added to namespace
			$.ajax({
				url: url,
				type: 'POST',
				context: this,
	...
	jQuery.extend({
		getData: function(url, data) { // getData method added to namespace
			var result;
			$.ajax({
				url: url,
				type: 'GET',
